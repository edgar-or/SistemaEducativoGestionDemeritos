...
public class EncargadoDAO {
    ...
    public void insertarEncargado(ModeloEncardoAlumno e) throws SQLException {
        String sql = "INSERT INTO encargado_alumno (nombre, apellido, dui, telefono, departamento, municipio, caserio, calle, distrito) "
                   + "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    public List<ModeloEncardoAlumno> listarEncargados() throws SQLException {
        ...
        String sql = "SELECT nombre, apellido, dui, telefono, departamento, municipio, caserio, calle, distrito FROM encargado_alumno";
        try (Connection con = Conexion.getConexion();
             PreparedStatement ps = con.prepareStatement(sql);
    ...
    public List<ModeloEncardoAlumno> buscarPorDui(int dui) throws SQLException {
        List<ModeloEncardoAlumno> lista = new ArrayList<>();
        String sql = "SELECT nombre, apellido, dui, telefono, departamento, municipio, caserio, calle, distrito FROM encargado_alumno WHERE dui = ?";
        try (Connection con = Conexion.getConexion();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, dui);
    ...
    public List<ModeloEncardoAlumno> buscarPorNombre(String nombre) throws SQLException {
        List<ModeloEncardoAlumno> lista = new ArrayList<>();
        String sql = "SELECT nombre, apellido, dui, telefono, departamento, municipio, caserio, calle, distrito FROM encargado_alumno WHERE nombre LIKE ? OR apellido LIKE ?";
        try (Connection con = Conexion.getConexion();
             PreparedStatement ps = con.prepareStatement(sql)) {
    ...
    public void modificarEncargado(ModeloEncardoAlumno e, int duiOriginal) throws SQLException {
        String sql = "UPDATE encargado_alumno SET nombre=?, apellido=?, telefono=?, departamento=?, municipio=?, caserio=?, calle=?, distrito=? WHERE dui=?";
        try (Connection con = Conexion.getConexion();
             PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setString(1, e.getNombre());
            ps.setString(2, e.getApelllido());
